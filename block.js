...
var Block = function(_type) {
    ...
    for (let xx = temp_matrix.length-1; xx >= 0; xx--) {
      for (let yy = temp_matrix[xx].length-1; yy >= 0; yy--) {
        ...
        if (temp_matrix[xx][yy] == 1) { // 1 = block / 0 = emty
          if (pos_x + xx + _x < 0 || pos_x + xx + _x > 9 || pos_y + yy + _y >= 20 || // walls (left || right || down)
          matrix[pos_x + xx + _x][pos_y + yy + _y] !== undefined) { // blocks in matrix (left || right || down)
            return true;
          }
        }
  ...
  this.pushInMatrix = function() {

    for (let xx = 0; xx < this.matrix.length; xx++) {
      for (let yy = 0; yy < this.matrix[xx].length; yy++) {

        if (this.matrix[xx][yy] == 1)
          matrix[(this.pos.x/block_size) + xx][(this.pos.y/block_size) + yy] = this.type;
